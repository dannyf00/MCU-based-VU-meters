...
#define _DLY_MS					(223)			//164 no optimization or 223 full optimization delay for PIC24F @ 4Mhz (8Mhz Crystal)
...
//delay_us(1000) = 10ms
//delay_us(1) = 10us
void delay_us(volatile unsigned int us) {            //delay a us
	delay(us);						//delay the lsb
	us = us >> 8;					//retain the msb
	while (us--) delay(0xff);		//delay the msb
}

void delay_ms(volatile unsigned int ms) {	//delay ms
